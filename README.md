...
This project recreates a small web infrastructure using Docker and Docker Compose on a Virtual Machine.
It includes three core services — NGINX, WordPress (with PHP-FPM), and MariaDB — each running in its own container, communicating through a custom Docker network, and sharing data via persistent volumes.

Below are my personal notes, debugging logs, and lessons learned during the project.
They reflect configuration details, common errors, and fixes that I thought you may find helpful when doing it.
...
- cosa superdivertida relacionada con el ultimo punto. aunque los scripts se ejecuten en el momento de creacion del contenedor, no los lee de los archivos como tal, el contenedor se crea con la informacion que haya acumulada en la imagen (creada por las instrucciones del dockerfile). Si creamos imagen + contenedor, luego eliminamos contenedor, modificamos entrypoint.sh y volvemos a crear contenedor sin volver a crear imagen, podemos tener  problemillas. El dockerfile en el momento de hacer la imagen tiene la info de los .sh y los agrupa en el snapshot de la imagen. aunque luego cambiemos ese .sh, no afecta a la informacion que ya tiene guardada la imagen en si. esto con docker solo solo tiene el remedio de volver a crear la imagen para actualizar el script y ocn docker-compose se puede usar 'docker compose build --no-cache'. imagino asi se vuelve a crear la imagen. 

- cuando ponemos ENTRYPOINT["ruta/a/script.sh"] en un Dockerfile, ese script se convierte en PID 1 (mientras se ejecuta), es el proceso que mantiene el contenedor vivo. Si dentro del script ponemos un exec '$@' o exec 'myslqd', estamos finalizando el script y lanzando como PID 1 el comando que sea. Eso es lo que hay que hacer siempre porque siempre queremos lanzar solo un programa por contenedor y que esté en primer plano. Yo antes en el script de mariadb tenía:
    ```bash
    mysqld &
        (configuraciones de roots, etc)
    wait mysqld
    ```

esto lanzaba mysqld como daemon, osea, en segundo plano, y luego el propio script se esperaba a que ese proceso acabase. Que como es un daemon, nunca acaba, a no ser que haya algún problema serio, se apague el contenedor, etc. Una de las consecuencias de hacer esto es que se quede colgado el proceso hijo que se crea dentro del script para ejecutar mysqld, además de que la señal de SIGTERM no se manda a MariaDB, sino a bash, lo cual dificulta identificar problemas con los logs y tal.

Por lo tanto, hay que poner siempre en el Dockerfile: o bien ENTRYPOINT[script] y CMD, o tener solo ENTRYPOINT[script] y luego un 'exec programaenprimerplano' dentro del script. 
Con nginx hay que especificar que queremos que se ejecute en primer plano con la flag 'daemon off', porque sino de manera normal lo que hace nginx es:
    1. Forks a master process (PID 1 temporarily)
    2. That master process spawns worker processes and then detaches (goes to background)
    3. It writes a PID file and exits the parent process
Así que hay que escribir: `CMD ["nginx", "-g", "daemon off;"]`.
Por qué la diferencia?
    > nginx was designed as a system daemon — meant to run under init/systemd and fork itself to background immediately.
    > mysqld was designed as a server process that can be started either manually or by init scripts, so it stays in the foreground by default and lets its controlling system handle backgrounding.
So in Docker, nginx must be told not to detach, while mysqld doesn’t need to be told anything.

Dejo escrito en el apartado de mariadb cómo hacer bien la inicializacion teniendo esto en cuenta.

## 2. Certificado OpenSL
